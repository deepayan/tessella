...
current.panel.limits <- function(vp = lattice.getStatus("current.viewport"))
{
    with(vp, list(xlim = xlim, ylim = ylim))
}
             ...
             vp = lattice.getStatus("current.viewport"))
{
    ...
    gp.line <- list(col = line.col, alpha = line.alpha,
                    lty = line.lty, lwd = line.lwd)
    gp.text <- list(col = text.col, cex = text.cex, alpha = text.alpha,
    ...
    ## axis.units will be currently like
    ## list(tick = list(x = 5),
    ##      pad1 = list(x = 5),
    ##      pad2 = list(x = 5))

    axis.settings <- trellis.par.get("axis.components")[[side]]
    tck.x <- tck * axis.settings$tck * axis.units$tick$x
    ## tck.unit.x <- tck * axis.settings$tck * axis.units$tick$x
    ## tck.unit <- unit(x = tck.unit.x, units = axis.units$tick$units)

    lab.x <- tck.x + axis.settings$pad1 * axis.units$pad1$x
    orient.factor <- if (outside) -1 else 1

    rescalevp <- function(xlim, ylim)
    {
        tviewport(vp,
                  x = vp$xlim[1], y = vp$ylim[1],
                  w = diff(vp$xlim), h = diff(vp$ylim),
                  xlim = xlim, ylim = ylim)
    }
 
    if (ticks && any(tck.x != 0))
        switch(side, 
               bottom = 
           {
               nvp <- rescalevp(xlim = vp$xlim, ylim = c(0, vp$h))
               tsegments(x0 = at[axid],
                         x1 = at[axid],
                         y0 = 0,
                         y1 = orient.factor * tck.x,
                         vp = nvp)
           },
               top = 
           {  
               nvp <- rescalevp(xlim = vp$xlim, ylim = c(0, vp$h))
               ## str(list("top", xlim = vp$xlim, ylim = c(0, vp$h), at = at[axid]))
               tsegments(x0 = at[axid],
                         x1 = at[axid],
                         y0 = nvp$ylim[2],
                         y1 = nvp$ylim[2] - orient.factor * tck.x,
                         vp = nvp)
           },
               left =
           {
               nvp <- rescalevp(xlim = c(0, vp$w), ylim = vp$ylim)
               ## str(list("left", xlim = c(0, vp$w), ylim = vp$ylim, at = at[axid]))
               tsegments(y0 = at[axid],
                         y1 = at[axid],
                         x0 = 0,
                         x1 = orient.factor * tck.x,
                         vp = nvp)
           },
               right =
           {
               ...
               tsegments(y0 = at[axid],
                         y1 = at[axid],
                         x0 = nvp$xlim[2],
                         x1 = nvp$xlim[2] - orient.factor * tck.x,
                         vp = nvp)
           })
    ...
    {
        {
            adj <-
                if (outside)
                    switch(side,
                           bottom = if (rot[1] == 0) c(0.5, 1) else c(1, 0.5),
                           top = if (rot[1] == 0) c(0.5, 0) else c(1, 0.5),
                           left = if (rot[2] == 90) c(0.5, 0) else c(1, 0.5),
                           right = if (rot[2] == 90) c(0.5, 1) else c(0, 0.5))
                else
                    switch(side,
                           bottom = if (rot[1] == 0) c(0.5, 0) else c(0, 0.5),
                           top = if (rot[1] == 0) c(0.5, 1) else c(1, 0.5),
                           left = if (rot[2] == 90) c(0.5, 1) else c(0, 0.5),
                           right = if (rot[2] == 90) c(0.5, 0) else c(1, 0.5))
        }
        switch(side,
               bottom =
           {
               nvp <- rescalevp(xlim = vp$xlim, ylim = c(0, vp$h))
               ttext(x = at[axid & keep.labels],
                     y = orient.factor * lab.x,
                     labels = labels[axid & keep.labels],
                     rot = rot[1],
                     check.overlap = check.overlap,
                     adj = adj,
                     col = text.col, cex = text.cex, alpha = text.alpha,
                     fontface = chooseFace(text.fontface, text.font),
                     fontfamily = text.fontfamily, lineheight = text.lineheight,
                     vp = nvp)
           },
               top =
           {
               nvp <- rescalevp(xlim = vp$xlim, ylim = c(0, vp$h))
               ttext(x = at[axid & keep.labels],
                     y = nvp$ylim[2] - orient.factor * lab.x,
                     labels = labels[axid & keep.labels],
                     rot = rot[1],
                     check.overlap = check.overlap,
                     adj = adj,
           ...
           {
               nvp <- rescalevp(xlim = c(0, vp$w), ylim = vp$ylim)
               ttext(y = at[axid & keep.labels],
                     x = orient.factor * lab.x,
                     labels = labels[axid & keep.labels],
                     rot = rot[2],
                     check.overlap = check.overlap,
                     adj = adj,
               ...
               right =
           {
               nvp <- rescalevp(xlim = c(0, vp$w), ylim = vp$ylim)
               ttext(y = at[axid & keep.labels],
                     x = nvp$xlim[2] - orient.factor * lab.x,
                     labels = labels[axid & keep.labels],
                     rot = rot[2],
                     check.overlap = check.overlap,
                     adj = adj,
