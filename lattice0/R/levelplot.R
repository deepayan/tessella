             ...
             vp = lattice.getStatus("current.viewport"))
{
    ...
    if (region) 
    {
        panel.rect(x = cx[idx],
                   ...
                   col = adjustcolorv(zcol, alpha.regions),
                   border = border.col,
                   lwd = border.lwd,
                   lty = border.lty,
                   vp = vp)
...
{
    stop("panel.levelplot.raster not supported")
    if (length(subscripts) == 0) return()
    regions <- trellis.par.get("regions")
    x.is.factor <- is.factor(x)
    y.is.factor <- is.factor(y)
    ...
    ## rasterImage(as.raster(zmat), 
    ##             xleft = xlow, ybottom = ylow,
    ##             xright = xhigh, ytop = yhigh,
