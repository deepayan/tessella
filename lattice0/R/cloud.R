             ...
             vp = lattice.getStatus("current.viewport"))
{
    ...
    if (shade)
    {
        ...
        pol.x <- numeric(polynum * 4); pol.x[] <- NA_real_
        pol.y <- numeric(polynum * 4); pol.y[] <- NA_real_
        ...
        wirePolygon <-
            ...
            function(xx, yy, misc)
            {
                ...
                if (!invalid)
                {
                    pol.x[4 * count + 1:3] <<- xx
                    pol.y[4 * count + 1:3] <<- yy
                    ...
                    if (count == polynum)
                    {
                        ...
                        panel.polygon(x = pol.x, y = pol.y,
                                      col = pol.fill,
                                      border = pol.col,
                                      lty = lty, lwd = lwd,
                                      alpha = pol.alpha, vp = vp)
                        count <<- 0
                    }
                }
            }
        ...
        .Call(wireframePanelCalculations2,
              as.double(x),
              as.double(y),
              as.double(z),
              as.double(rot.mat),
        ...
        if (count > 0)
        {
            panel.polygon(x = pol.x[1:(count * 4)], y = pol.y[1:(count * 4)],
                          col = pol.fill,
                          border = pol.col,
                          lty = lty, lwd = lwd,
                          alpha = pol.alpha, vp = vp)
    ...
    else  ## no shade
    {
        ...
        pol.x <- numeric(polynum * 5); pol.x[] <- NA_real_
        pol.y <- numeric(polynum * 5); pol.y[] <- NA_real_
        ...
        wirePolygon <-
            ...
            function(xx, yy, misc)
            {
                ...
                if (!invalid)
                {
                    pol.x[5 * count + 1:4] <<- xx
                    pol.y[5 * count + 1:4] <<- yy
        ...
        if (count > 0)
        {
            panel.polygon(x = pol.x[1:(count * 5)], y = pol.y[1:(count * 5)],
                          col = pol.fill,
                          border = pol.col,
                          lty = lty, lwd = lwd,
                          alpha = pol.alpha, vp = vp)
...
##                          col.regions = col.regions,
        ...
        ## slightly different from xyplot etc, in that rot can be
        ## supplied in the *lab lists

        plotLabel <- function(x, y, label = NULL, ...)
        {
            if (is.null(label)) return(invisible())
            ttext(x, y, label, ..., vp = vp)
        }
        do.call(plotLabel, c(list(x = tlabs[1, 1], y = tlabs[2, 1]), xlab))
        do.call(plotLabel, c(list(x = tlabs[1, 2], y = tlabs[2, 2]), ylab))
        do.call(plotLabel, c(list(x = tlabs[1, 3], y = tlabs[2, 3]), zlab))
