             ...
             vp = lattice.getStatus("current.viewport"))
{
    ...
    xscale <- current.panel.limits(vp)$xlim
    tlines(x = xscale[1] + diff(xscale) * c(0.05, 0.95),
           y = c(0,0),
           col = adjustcolorv(border, alpha), lty = lty, lwd = lwd, vp = vp)
    ...
    if (length(x) > 0)
    {
        ...
        if (nb > 1)
        {
            trect(xleft = breaks[-nb], xright = breaks[-nb] + diff(breaks),
                  ybottom = 0, ytop = y,
