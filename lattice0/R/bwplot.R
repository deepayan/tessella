             ...
             ...)
{
    ...
    if (horizontal)
    {
        ...
        if (is.null(groups))
        {
            ...
            panel.rect(xleft = rep(origin, length(y)),
                       y = y,
...
##             nvals <- length(vals)
            ...
            for (i in unique(y))
            {
                ...
                if (nok > 0)
                {
                    xl <- cumsum(c(0, x[ok][ord][pos][-nok]))
                    panel.rect(xleft = xl,
                               y = rep(i, nok),
                               ...
                               xright = xl + x[ok][ord][pos])
                }
                neg <- x[ok][ord] < 0
                nok <- sum(neg, na.rm = TRUE)
                if (nok > 0)
                {
                    xl <- cumsum(c(0, x[ok][ord][neg][-nok]))
                    panel.rect(xleft = xl,
                               y = rep(i, nok),
...
##             nvals <- length(vals)
            ...
            for (i in unique(y))
            {
                ...
                panel.rect(xleft = rep(origin, nok), 
...
##             nvals <- length(vals)
            ...
            for (i in unique(x))
            {
                ...
                if (nok > 0)
                {
                    yb <- cumsum(c(0, y[ok][ord][pos][-nok]))
                ...
                if (nok > 0)
                {
                    yb <- cumsum(c(0, y[ok][ord][neg][-nok]))
                    panel.rect(x = rep(i, nok),
                               ybottom = yb,
                               col = col[groups[ok][ord][neg]],
                               border = border[groups[ok][ord][neg]],
             ...
             vp = lattice.getStatus("current.viewport"))
{
    ...
    if (horizontal)
    {
        for (i in seq_along(levels.fos))
        {
            ...
            {
                svp <- tviewport(vp, y = levels.fos[i] - height / 2, h = height,
                                 ylim = c(max.d[i] * c(-1, 1)),
                                 xlim = cpl$xlim)
                panel.polygon(x = c(dx.list[[i]], rev(dx.list[[i]])),
                              y = c(dy.list[[i]], -rev(dy.list[[i]])),
                              col = col, border = border, lty = lty,
                              lwd = lwd, alpha = alpha, vp = svp)
            }
        }
    }
    else
    {
        for (i in seq_along(levels.fos))
        {
            if (is.finite(max.d[i]))
            {
                svp <- tviewport(vp, x = levels.fos[i] - height / 2, w = height,
                                 xlim = c(max.d[i] * c(-1, 1)),
                                 ylim = cpl$ylim)
                panel.polygon(y = c(dx.list[[i]], rev(dx.list[[i]])),
