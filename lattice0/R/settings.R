...
show.settings <- function(x = NULL)
{
    ...
    panel.box <- function()
    {
        panel.fill(col = "transparent",
                   border = adjustcolorv(par.box$col, par.box$alpha),
                   lty = par.box$lty,
                   lwd = par.box$lwd)
    }
               ...
               switch(as.character(x),
                      ...
                      "strip.background" = {
                          ...
                          panel.rect(y = ppoints(len), height = 0.5 / len,
                                     ...
                                     col = adjustcolorv(strip.background$col, strip.background$alpha),
                      ...
                      "strip.shingle" = {
                          ...
                          panel.rect(y = ppoints(len), height = 0.5 / len,
                                     xleft = cpl$xlim[1], xright = cpl$xlim[2],
                                     col = adjustcolorv(strip.shingle$col, strip.shingle$alpha),
                                     border = "transparent")
                      },
                      "dot.[symbol, line]" = {
...
.defaultLatticeOptions <- function()
    list(backend = graphics_primitives,
         save.object = TRUE,
         panel.error = NULL, ## FIXME: (for testing; revert eventually) "panel.error",
         ...
         ## In the tessella world, the only unit that makes sense is
         ## context-native (usually pixel).  We will still keep a list
         ...
         list(top.padding = list(x = 5),
              main = list(x = 0),
              main.key.padding = list(x = 5),
              key.top = list(x = 0),
              xlab.top = list(x = 0),
              key.axis.padding = list(x = 5),
              axis.top = list(x = 0),
              strip = list(x = 20),
              panel = list(x = -1),
              axis.panel = list(x = 0),
              between = list(x = 10),
              axis.bottom = list(x = 0),
              axis.xlab.padding = list(x = 5),
              xlab = list(x = 0),
              xlab.key.padding = list(x = 5),
              key.bottom = list(x = 0),
              key.sub.padding = list(x = 5),
              sub = list(x = 0),
              bottom.padding = list(x = 5)),

         layout.widths =
         list(left.padding = list(x = 5),
              key.left = list(x = 0),
              key.ylab.padding = list(x = 5),
              ylab = list(x = 0),
              ylab.axis.padding = list(x = 5),
              axis.left = list(x = 0),
              axis.panel = list(x = 0),
              strip.left = list(x = 20),
              panel = list(x = -1),
              between = list(x = 10),
              axis.right = list(x = 0),
              axis.key.padding = list(x = 5),
              ylab.right = list(x = 0),
              key.right = list(x = 0),
              right.padding = list(x = 5)),
